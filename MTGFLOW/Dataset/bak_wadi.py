...
def loader_WADI(root, batch_size, window_size, stride_size,train_split,label=False):
    ...
    # data = pd.read_csv("Dataset/input/WADI_14days.csv",sep=",",skiprows=4)#, nrows=1000)
    # data = pd.read_csv("Dataset/input/WADI_14days.csv",sep=",",skiprows=4, nrows=1000)
    data = pd.read_csv("Dataset/input/WADI_14days.csv",sep=",",skiprows=4, nrows=1000)
    # data = pd.read_csv("Data/input/WADI_attackdata.csv",sep=",")#, nrows=1000)
    labels=[]
    ...
    for index, row in data.iterrows():
        ...
        if date_obj==datetime.strptime('10/9/2017', '%m/%d/%Y'):
            if time_obj>=datetime.strptime('7:25:00.000 AM', '%I:%M:%S.%f %p') and time_obj<=datetime.strptime('7:50:16.000 PM', '%I:%M:%S.%f %p'):
                labels.append('Attack')
                continue
...
class WADI_dataset(Dataset):
    ...
    def preprocess(self, df, label):
        ...
        '''
        对每个窗口范围内的原始标签求和
        如果和为0,说明窗口内所有点都是正常的(标签为0),则该窗口标签为0
        如果和大于0,说明窗口内存在异常点(标签为1),则该窗口标签为1
        '''
    ...
    def __len__(self):
        ...
        return length
    '''
    它是PyTorch数据加载机制中的关键组件,实现了数据从存储到模型输入的转换过程
    '''
    def __getitem__(self, index):
        ...
        # 将窗口数据reshape为[window_size, num_features, 1]的形状
        data = self.data[start:end].reshape([self.window_size,-1, 1])
        # 转置为[num_features, window_size, 1]并返回对应的标签和索引
