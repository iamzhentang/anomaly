...
class MAF(nn.Module):
    def __init__(self, n_blocks, n_sensor, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu', input_order='sequential', batch_norm=True, mode = 'rand'):
        ...
        for i in range(n_blocks):
            '''
            创建了一系列模块（modules），
            每个模块都是一个ADE（Autoregressive Density Estimator）层，用于构建自动回归模型
        ...
        '''构造模型：MAF模型通过堆叠放多个ADE层来构造。
        每个ADE层都是一个自动回归网络，它使用掩码来确保输出只依赖于输入的一部分。
        ADE层的输出作为下一个ADE层的输入。
        vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv'''
    ...
    def forward(self, x, y=None):
        '''在前向传播过程中，输入数据通过ADE层的序列进行处理。
        每个ADE层都会对输入数据应用非线性变换，并计算变换的对数雅可导数（log_abs_det_jacobian）。
        这些雅可导数用于计算最终的对数概率。
        vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv'''
        return self.net(x, y)
    ...
    '''计算对数概率：MAF模型的log_prob方法计算输入数据的对数概率。
    它通过前向传播得到变换后的输出，并使用基本分布的对数概率和累积的雅可导数来计算总的对数概率。
    vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv'''
